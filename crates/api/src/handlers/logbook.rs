...
use crate::models::{LogbookEntry, UpsertRecordRequest};
use crate::state::AppState;

pub async fn list_logbook(state: web::Data<AppState>, req: HttpRequest) -> Result<HttpResponse, ApiError> {
    let actor = require_user(&req, &state).await?;
    let entries = sqlx::query_as::<_, LogbookEntry>(
        r#"
...
pub async fn create_logbook(
    ...
    let title = payload
        ...
        .unwrap_or_else(|| "Logbook Entry".to_string());

    let created = sqlx::query_as::<_, LogbookEntry>(
        r#"
        INSERT INTO logbook_entries (id, user_id, title, payload, created_at, updated_at)
...
pub async fn get_logbook(
    ...
    let actor = require_user(&req, &state).await?;
    let entry = sqlx::query_as::<_, LogbookEntry>(
        r#"
        SELECT id, user_id, title, payload, created_at, updated_at
        FROM logbook_entries
        WHERE id = $1
...
pub async fn update_logbook(
    ...
    let actor = require_user(&req, &state).await?;
    let before = sqlx::query_as::<_, LogbookEntry>(
        r#"
        SELECT id, user_id, title, payload, created_at, updated_at
        FROM logbook_entries
        WHERE id = $1
    ...
    let updated = sqlx::query_as::<_, LogbookEntry>(
        r#"
...
pub async fn delete_logbook(
    ...
    sqlx::query("DELETE FROM logbook_entries WHERE id = $1")
        .bind(before.id)
        .execute(&state.pool)
        .await?;
